...
if($b == "add") {
						...
						function load_account_fiels(value) {
                            ...
                            if($FormBTN == "deposit_gateway") {
                                ...
                                if(empty($name)) {
                                    ...
                                    $insert = $db->query("INSERT gateways (name,currency,type,a_field_1,a_field_2,a_field_3,external_gateway,status,min_amount,max_amount) VALUES ('$name','$currency','1','$a_field_1','$a_field_2','$a_field_3','1','$status','$min_amount','$max_amount')");
                            ...
                            <form action="" method="POST">
                                <div class="form-group">
                                    <label>Gateway</label>
                                    <input type="text" class="form-control" name="name" onchange="load_account_fiels(this.value);">
                                </div>
                                <div class="form-group">
	...
	if($query->num_rows==0) { header("Location: ./?a=manual_deposit"); }
	$row = $query->fetch_assoc();
	?>
                        ...
                        <div class="card-body">
                            ...
                            if($FormBTN == "info") {
                                if($row['name'] !== "Bank Transfer") {
                                    ...
                                    $update = $db->query("UPDATE gateways SET a_field_1='$a_field_1',a_field_2='$a_field_2',a_field_3='$a_field_3',a_field_4='$a_field_4',a_field_5='$a_field_5',currency='$currency' WHERE id='$row[id]'");
                                ...
                                $update = $db->query("UPDATE gateways SET min_amount='$min_amount', max_amount='$max_amount' WHERE id='$row[id]'");
                                
                                $status = protect($_POST['status']);
                                $update = $db->query("UPDATE gateways SET status='$status',currency='$currency' WHERE id='$row[id]'");
                                $query = $db->query("SELECT * FROM gateways WHERE id='$row[id]'");
                                $row = $query->fetch_assoc();
           ...
           <div class="col-md-12">
			...
			if(isset($_GET['confirm'])) {
				...
				echo '<a href="./?a=deposit_methods&b=delete&id='.$row['id'].'&confirm=1" class="btn btn-success"><i class="fa fa-check"></i> Yes</a>&nbsp;&nbsp;
					<a href="./?a=manual_deposit" class="btn btn-danger"><i class="fa fa-times"></i> No</a>';
			}
			?>
		</div>
	</div>
					...
					<div class="card">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                $statement = "gateways WHERE external_gateway='1'";
                                ...
                                if($query->num_rows>0) {
                                    while($row = $query->fetch_assoc()) {
                                        ...
                                        <tr>
                                            ...
                                            ?></td>
                                            <td>
                                                <a href="./?a=manual_deposit&b=edit&id=<?php echo filter_var($row['id'], FILTER_SANITIZE_STRING); ?>" title="Edit"><span class="badge badge-primary"><i class="fa fa-pencil"></i> Edit</span></a> 
                                                <a href="./?a=manual_deposit&b=delete&id=<?php echo filter_var($row['id'], FILTER_SANITIZE_STRING); ?>" title="Delete"><span class="badge badge-danger"><i class="fa fa-trash"></i> Delete</span></a>
                                            </td>
                                        </tr>
                                        <?php
                                    }
